...
let nextId = 41;
let nextSectionId = 1;
const PRODUCTS_STORAGE_KEY = "luxora_products";
const SITE_SETTINGS_KEY = "luxora_site_settings";
let siteSettings = null;
let nextSocialId = 1;
let nextSliderId = 1;
const NEWSLETTER_SUBSCRIBERS_KEY = "luxora_newsletter_subscribers";
const NEWSLETTER_EMAIL_KEY = "luxora_newsletter_email";
const USERS_KEY = "luxora_users";
const ORDERS_KEY = "luxora_orders";
const EMAIL_TEMPLATES_KEY = "luxora_email_templates";
const EMAIL_OUTBOX_KEY = "luxora_email_outbox";
const EMAIL_MANUAL_RECIPIENTS_KEY = "luxora_email_manual_recipients";
const EMAIL_SUPPRESSED_KEY = "luxora_email_suppressed";
const VIEWERS_MODE_KEY = "luxora_viewers_mode";
const PAGE_META_KEY = "luxora_page_meta";
const PAGE_CONTENT_KEY = "luxora_page_content";

(function () {
  if (window.__luxoraMediaProtectionEnabled) return;
  window.__luxoraMediaProtectionEnabled = true;
...
const DEFAULT_EMAIL_TEMPLATES = [
  {
    id: 1,
    name: "Welcome to Luxora Circle",
    subject: "Welcome to the Luxora Circle",
  ...
  },
  {
    id: 5,
    name: "Order Update",
    subject: "Your Luxora order update",
  ...
  },
  {
    ...
    subject: "Shipping update from Luxora",
  ...
  },
  {
    ...
    subject: "Holiday gift picks from Luxora",
  ...
  },
  {
    ...
    subject: "Your Luxora monthly roundup",
...
function calculateDashboardMetrics() {
  ...
  let orders = JSON.parse(localStorage.getItem("luxora_orders")) || [];
...
$(document).ready(function () {
  const config = window.LuxoraAdminConfig || {};
  const authKey = "luxora_admin_auth";
  const emailKey = "luxora_admin_email";
  const passwordKey = "luxora_admin_password";
  const resetKeyStorage = "luxora_admin_reset_key";
  const defaultEmail = (
    config.adminEmailDefault || "luxora.ahmer@gmail.com"
  ).toLowerCase();
  const defaultPassword = config.adminPasswordDefault || "Luxora@2026";
  const defaultResetKey = config.resetKey || "LUXORA-RESET-2026";
...
function displayRecentOrders() {
  ...
  orders.forEach((order, idx) => {
    ...
    row.innerHTML = `
            ...
            <td class="py-3 text-light fw-semibold">₹${order.total.toLocaleString()}</td>
            <td class="pe-4 py-3">
                          ...
                          .map((item, i) => {
                            ...
                            <div style="background-color: #1a1a1a; padding: 12px; border-radius: 4px; border: 1px solid #444; display: flex; gap: 12px;">
                                ...
                                <div style="flex: 1;">
                                    ...
                                    <p class="text-secondary mb-0" style="font-size: 0.85rem;">Price: <strong class="text-orange">₹${price.toLocaleString()}</strong> × <strong>${qty}</strong> = <strong class="text-orange">₹${lineTotal.toLocaleString()}</strong></p>
                                </div>
                            </div>
                        `;
                          })
                ...
                <div style="background-color: #2a2a2a; padding: 20px; border-radius: 6px;">
                    <div class="row mb-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <p class="text-secondary mb-1"><strong class="text-light">Subtotal:</strong>₹ ${order.subtotal.toLocaleString()}</p>
                            <p class="text-secondary mb-1"><strong class="text-light">Shipping:</strong>₹ ${order.shipping}</p>
                            <p class="text-orange fw-bold"><strong>Total:</strong>₹ ${order.total.toLocaleString()}</p>
                        ...
                        ${order.items
                          .map((item, i) => {
                            ...
                            <div style="background-color: #1a1a1a; padding: 12px; border-radius: 4px; border: 1px solid #444; display: flex; gap: 12px;">
                                ...
                                <div style="flex: 1;">
                                    <p class="text-light fw-semibold mb-1" style="font-size: 0.95rem;">${item.name}</p>
                                    <p class="text-secondary mb-0" style="font-size: 0.9rem;"><strong>Unit Price:</strong>₹ ${price.toLocaleString()} | <strong>Quantity:</strong> ${qty} | <strong class="text-orange">Total:₹ ${lineTotal.toLocaleString()}</strong></p>
                                </div>
                            </div>
                        `;
                          })
...
function displayAllCustomers() {
  ...
  customers.forEach((customer) => {
    ...
    row.innerHTML = `
            ...
            <td class="pe-4 py-3 text-light fw-semibold">₹${customer.totalSpent.toLocaleString()}</td>
        `;
    tbody.appendChild(row);
  });
}
...
function buildDefaultSiteSettings() {
  return {
    brand: {
      name: "LUXORA",
      logo: "frontend/assets/images/logo/luxora-logo.webp",
      favicon: "frontend/assets/images/favicon/luxora-watches-icon.ico",
    },
    contact: {
      address: "Barrage Colony, HYD, PK",
      email: "luxora.ahmer@gmail.com",
    ...
    socialLinks: [
      {
        id: 1,
        label: "Facebook",
        url: "https://www.facebook.com/luxora.ahmer",
        icon: "bi bi-facebook",
      },
      {
        id: 2,
        label: "X",
        url: "https://x.com/luxora_ahmer",
        icon: "bi bi-twitter",
      },
      {
        id: 3,
        label: "Instagram",
        url: "https://www.instagram.com/luxora.ahmer",
