...
function applyBrandSettings(brand) {
  ...
  if (logo) {
    ...
    document
      .querySelectorAll(".navbar-logo, .offcanvas-logo")
      .forEach((img) => {
        if (!img.alt || img.alt.toLowerCase().includes("luxora")) {
          img.alt = `${name} Logo`;
        }
      });
  }
...
function updateMetaForBrand(brandName) {
  if (!brandName) return;
  const replaceBrand = (value) => value.replace(/\bLuxora\b/gi, brandName);
...
function replaceBrandTextNodes(root, brandName) {
  ...
  while (node) {
    ...
    if (!parent || !skipTags.has(parent.tagName)) {
      if (/\bLuxora\b/i.test(node.nodeValue)) {
        node.nodeValue = node.nodeValue.replace(/\bLuxora\b/gi, brandName);
      }
    }
                ...
                <div class="card-body">
                    <div class="d-flex justify-content-between flex-wrap gap-2">
                        ...
                        <div class="text-end">
                            ...
                            <p class="text-white fw-bold mt-2">₹${order.total.toLocaleString()}</p>
                        </div>
                    </div>
                    <div class="mt-3">
                        ${order.items
            ...
            (item) => `
                            <div class="d-flex align-items-center gap-2 mb-2">
                                ...
                                <div>
                                    ...
                                    <div class="text-secondary small">Qty: ${item.quantity} · ₹${item.price}</div>
                                </div>
                            </div>
                        `,
          )
...
function initAccountPage() {
  ...
  const user = window.LuxoraAuth?.getCurrentUser?.();
  if (!user) {
  ...
  if (profileImage.length) {
    const storedImage = localStorage.getItem(
      `luxora_profile_image_${user.email}`,
    );
    if (storedImage) {
  ...
  profileForm.on("submit", function (e) {
    ...
    const result = window.LuxoraAuth.updateUser(user.email, updates);
    if (!result.ok) {
  ...
  passwordForm.on("submit", function (e) {
    ...
    const result = window.LuxoraAuth.updatePassword(
      user.email,
      currentPassword,
      newPassword,
    );
  ...
  profilePictureForm.on("submit", function (e) {
    ...
    reader.onload = function (evt) {
      ...
      if (result) {
        localStorage.setItem(`luxora_profile_image_${user.email}`, result);
...
$(document).ready(function () {
  ...
  function updateSummary(subtotal, totalItems) {
    ...
    $("#cart-shipping").html(`
            <span style="text-decoration: line-through; color: #b0b0b0;">₹1,000</span> 
  ...
  function fetchProductsData() {
    ...
    return $.ajax({
      ...
      .catch(() => {
        console.warn("API unavailable, falling back to products.json");
        return $.ajax({
          url: "products.json",
          method: "GET",
          dataType: "json",
  ...
  function updateProductMeta(product) {
    if (!product) return;
    const title = `${product.name} | Luxora`;
    const description =
      product.description ||
      "Discover premium Luxora watches and accessories.";
    const canonicalUrl = `https://luxora.com/products.html?${product.id != null ? `id=${product.id}` : `name=${encodeURIComponent(product.name)}`}`;
                ...
                <div class="row g-4 align-items-center">
                    ...
                    <div class="col-lg-7">
                        ...
                        <div class="price-stack">
                            ...
                            ${salePrice ? `<span class="price-sale">₹${salePrice}</span>` : ""}
                        </div>
                        <div class="spec-grid">
                            <div class="spec-card">
                            ...
                            <div class="spec-card">
                                ...
                                <span class="spec-value">${product.sectionName || "Luxora"}</span>
                            </div>
                            <div class="spec-card">
  ...
  function renderShareButtons(product) {
    ...
    const url = `https://luxora.com/products.html?${product.id != null ? `id=${product.id}` : `name=${encodeURIComponent(product.name)}`}`;
    const text = encodeURIComponent(`Check out ${product.name} on Luxora.`);
    container.html(`
        ...
        .map(
          (p) => `
            <button type="button" class="suggestion-item" data-name="${p.name}">
                ...
                <span class="suggestion-price">₹${parseInt(p.salePrice).toLocaleString()}</span>
            </button>
        `,
        )
        .join(""),
  ...
  $("#completeCheckoutBtn").on("click", function () {
    const currentUser = window.LuxoraAuth?.getCurrentUser?.();
    if (!currentUser) {
    ...
    const selectedPayment = $("#paymentMethod").val();
    let paymentMethodLabel = "Cash on Delivery (COD)";
    let upiTxnId = "";

    if (selectedPayment === "upi") {
      upiTxnId = $("#upiTransactionId").val().trim();
      if (!upiTxnId) {
        showNotif("Please enter UPI Transaction / UTR number after payment.", "warning");
        $("#upiTransactionId").focus();
        return;
      }
...
function loadProductsBySection(sectionId, containerId) {
  ...
  if (stored) {
    try {
      ...
      console.warn("Invalid local products data, fallback to API");
    }
  }
                ...
                <div class="card-body">
                    ...
                    <div class="mb-3">
                        <span class="original-price"
                            style="text-decoration: line-through; color: #b0b0b0;">₹${originalPrice}</span>
                        <span class="sale-price"
                            style="color: #ff6600; font-weight: bold; margin-left: 5px;">₹${salePrice}</span>
                    </div>
...
function initNewsletterModal() {
  ...
  const dismissed = localStorage.getItem("luxora_newsletter_dismissed");
  const dismissedAt = parseInt(
    localStorage.getItem("luxora_newsletter_dismissed_at") || "0",
    10,
  );
  const cooldownMs = 7 * 24 * 60 * 60 * 1000;
  if (dismissed && dismissedAt && Date.now() - dismissedAt < cooldownMs) return;
  if (dismissed && !dismissedAt) {
    localStorage.removeItem("luxora_newsletter_dismissed");
  }
  ...
  $("#newsletterForm").on("submit", function (e) {
    ...
    localStorage.setItem("luxora_newsletter_dismissed", "true");
    localStorage.setItem(
      "luxora_newsletter_dismissed_at",
      String(Date.now()),
    );
...
function upsertNewsletterSubscriber(email, source) {
  ...
  const key = "luxora_newsletter_subscribers";
  const list = (() => {
  ...
  localStorage.setItem("luxora_newsletter_email", normalized);
  return true;
}
    ...
    result.html(`
            <div class="card product-card">
                <div class="card-body">
                    ...
                    <p class="text-secondary mb-2"><strong>Total:</strong> ₹${match.total.toLocaleString()}</p>
...
const COMPARE_KEY = "luxora_compare";
            ...
            <div class="card product-card mb-3">
                <div class="card-body d-flex align-items-center gap-3">
                    ...
                    <div class="flex-grow-1">
                        <h3 class="product-name mb-1">${item.name}</h3>
                        <div class="mb-2">
                            ${originalPrice ? `<span class="original-price" style="text-decoration: line-through; color: #b0b0b0;">₹${originalPrice}</span>` : ""}
                            ${salePrice ? `<span class="sale-price" style="color: #ff6600; font-weight: bold; margin-left: 6px;">₹${salePrice}</span>` : ""}
                        </div>
                        <div class="d-flex gap-2">
...
function renderComparePage() {
  ...
  const rows = [
    ...
    },
    {
      ...
      key: "price",
      render: (item) =>
        `₹${item.price != null ? parseInt(item.price).toLocaleString() : "N/A"}`,
    },
    {
      label: "Sale Price",
      key: "salePrice",
      render: (item) =>
        `₹${item.salePrice != null ? parseInt(item.salePrice).toLocaleString() : "N/A"}`,
    },
    {
      label: "Movement",
      key: "movement",
